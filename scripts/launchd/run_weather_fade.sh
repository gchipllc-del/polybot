...
#
# Usage:  run_weather_fade.sh scan [--thr 0.03]   (book new day-ahead fades)
#         run_weather_fade.sh settle              (resolve + P&L open paper trades)
# Forwards ALL args to weather_fade.py, so the plist can pass flags like --thr.
#
...
echo "=== $(date -u +%Y-%m-%dT%H:%M:%SZ) weather_fade $* ===" >> "$PROJECT_ROOT/logs/weather_fade.log"
exec python scripts/weather_fade.py "$@" >> "$PROJECT_ROOT/logs/weather_fade.log" 2>&1
