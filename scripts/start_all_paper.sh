#!/bin/bash
# Start ALL polybot paper-trading sleeves on this host.
#
# For each sleeve: run ONE cycle as a smoke test (paper only, never real
# orders), then install + (re)load its launchd cron so it keeps running.
# A sleeve whose smoke test fails (e.g. an API not allowlisted) is skipped
# with a warning; the others still come up.
#
# Covers the four signal/mechanical sleeves (no LLM API cost):
#   - btc_arb        (Polymarket BTC arb,    120s)
#   - btc_5min       (Polymarket 5-min,       60s)
#   - kalshi_15min   (Kalshi 15-min BTC,      60s)
#   - kalshi_weather (Kalshi hourly temp,    300s)
#
# It does NOT start the general LLM `trade` pipeline (that one makes paid
# forecaster API calls) — start that separately if you want it.
#
...
SLEEVES=(
  "com.jesse.polybot.btc_arb|btc-arb-monitor"
  "com.jesse.polybot.btc_5min|btc-5min-monitor"
...
echo "    $PY main.py btc-5min-paper-report"
echo "    $PY main.py btc-arb-paper-report"
echo "  Dashboards: :5050 main · :5053 crypto · :5054 weather"
echo
