...
2. **Pull state from these sources (in order):**
   ...
   - Todo/task state — any in-progress or pending tasks.
   - Background processes you started with `run_in_background` — shell/task IDs are load-bearing for the next agent.
   - Files created or modified this session — you know what you touched; don't grep to re-discover.
   - Memory or notes files written or updated (e.g. `~/.claude/projects/<project>/memory/`, or the repo's own docs/ if you wrote there).
   - Branch + PR state if you pushed (branch name, PR number/URL) — the next agent needs these to continue the same PR.
   - Unresolved questions — things you asked the user that never got a clear answer, or things the user asked that got deflected.
3. **Do NOT audit the filesystem.** This is synthesis of what happened in THIS session. No broad `git log` or `Glob` sweeps to rediscover work. If you didn't touch it this session, it doesn't belong here. (Naming the branch/PR you actually pushed to is fine — that's session state, not a filesystem audit.)
...
- <decision or change> — <why, and where it lives (absolute path if a file; commit SHA if pushed)>
- ...

## Key files for next session
- `<absolute path>` — <why the next agent should read this first>
- Plan file: `<path>` (if a plan drove the session)
- Memory/notes files touched: `<paths>` (if any)

## Running state
- Background processes: <shell/task IDs + what they are + how to kill> — or "none"
- Branch / PR: <branch name + PR number/URL> — or "none"
...
5. **No emojis, no hype, no "great job" summaries.** Terse and concrete — paths, commands, shell/task IDs, branch/PR, decisions. Match the tone of a seasoned engineer handing off at end-of-shift.
6. **Background process IDs and the branch/PR are critical.** If you started any `run_in_background` shells/tasks, their IDs must appear in "Running state" with how to stop them. If you pushed to a branch/PR, name it — the next agent cannot continue the same PR otherwise.
